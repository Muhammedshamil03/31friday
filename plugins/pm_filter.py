...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    ...
    if imdb and imdb.get('poster'):
        try:
            hehe = await message.reply_sticker(sticker=('CAACAgUAAxkBAAEIYyNkJceK3SQ_CqXgyDu_qNjBPN_qVQACpwADyJRkFGCmdrVn5RydLwQ'), reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(IMDB_DELET_TIME)
            await hehe.delete()            
        except (MediaEmpty, PhotoInvalidDimensions, WebpageMediaEmpty):
            pic = imdb.get('poster')
            poster = pic.replace('.jpg', "._V1_UX360.jpg")
            hmm = await message.reply_sticker(photo='CAACAgUAAxkBAAEIYyNkJceK3SQ_CqXgyDu_qNjBPN_qVQACpwADyJRkFGCmdrVn5RydLwQ', reply_markup=InlineKeyboardMarkup(btn))           
            await asyncio.sleep(IMDB_DELET_TIME)
            await hmm.delete()            
